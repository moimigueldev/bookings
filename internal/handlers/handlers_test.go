	...
	for _, e := range theTests {
		if e.method == "GET" {
			...
			if response.StatusCode != e.expectedStatusCode {
				t.Errorf("for %s, expected the %d, but got %d", e.name, e.expectedStatusCode, response.StatusCode)
			}
		} else {
			values := url.Values{}
			...
			if res.StatusCode != e.expectedStatusCode {
				t.Errorf("for %s, expected the %d, but got %d", e.name, e.expectedStatusCode, res.StatusCode)
			}
		}
	}
}
